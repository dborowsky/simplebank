postgres:
	docker run -name udemy -p 5432:5432 -e POSTGES_USER=root -e POSTGRES_PASSWORD=secret -d postgres:14-alpine
createdb:
...
server:
	go run main.go		
.PHONY: postgres createdb dropddb migrateup migratedown sqlc server